...
export function UsersTab() {
  ...
  const load = async () => {
    ...
    if (profR.error) toast.error(profR.error.message);
    ...
    const resumeStats = new Map<string, { count: number; best: number | null }>();
    ...
    const enriched: UserRow[] = (profR.data ?? []).map((p) => ({
      id: p.id,
              ...
              return (
                <TableRow key={u.id}>
                  ...
                  <TableCell className="text-sm">
                    {u.best_score !== null ? (
                      <span className="font-medium">{u.best_score}</span>
                    ) : (
