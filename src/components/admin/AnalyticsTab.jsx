...
import { FileText, Briefcase, Users, Gauge, Loader2, TrendingUp, Sparkles, } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
function StatCard({ icon: Icon, label, value, hint, }) {
    return (<div className="rounded-2xl border border-border p-5" style={{ background: "var(--gradient-card)" }}>
...
export function AnalyticsTab() {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (resR.error)
                toast.error(resR.error.message);
            setResumes((resR.data ?? []));
    ...
    const stats = useMemo(() => {
        ...
        for (let i = 13; i >= 0; i--) {
            ...
            days[days.length - 1].avg = dayScored.length
                ? Math.round(dayScored.reduce((s, r) => s + (r.ats_score ?? 0), 0) / dayScored.length)
                : 0;
        }
    ...
    return (<div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard icon={Briefcase} label="Jobs" value={jobsCount} hint={`${activeJobs} active`}/>
        <StatCard icon={FileText} label="Resumes uploaded" value={resumes.length} hint={`${appsCount} applications`}/>
        <StatCard icon={Gauge} label="Average ATS score" value={stats.scoredCount ? `${stats.avg}` : "—"} hint={stats.scoredCount
            ? `from ${stats.scoredCount} analyzed`
            : "no resumes scored yet"}/>
      </div>

      {empty ? (<div className="rounded-2xl border border-border border-dashed p-12 text-center" style={{ background: "var(--gradient-card)" }}>
          ...
          <div className="rounded-2xl border border-border p-6" style={{ background: "var(--gradient-card)" }}>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={stats.days} margin={{ left: -10, right: 10, top: 10 }}>
                ...
                <YAxis yAxisId="right" orientation="right" domain={[0, 100]} tick={{ fontSize: 11, fill: "var(--muted-foreground)" }}/>
                ...
                <Line yAxisId="left" type="monotone" dataKey="resumes" stroke="var(--primary)" strokeWidth={2} dot={{ r: 3 }} name="Resumes"/>
                <Line yAxisId="right" type="monotone" dataKey="avg" stroke="var(--accent)" strokeWidth={2} dot={{ r: 3 }} name="Avg ATS"/>
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="rounded-2xl border border-border p-6" style={{ background: "var(--gradient-card)" }}>
              ...
              {stats.topSkills.length === 0 ? (<p className="text-sm text-muted-foreground">
                  No skills extracted yet.
                </p>) : (<ResponsiveContainer width="100%" height={260}>
                  <BarChart data={stats.topSkills} layout="vertical" margin={{ left: 0, right: 10 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" horizontal={false}/>
                    <XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: "var(--muted-foreground)" }}/>
                    <YAxis type="category" dataKey="skill" width={100} tick={{ fontSize: 11, fill: "var(--muted-foreground)" }}/>
                    <Tooltip contentStyle={{
                    background: "var(--popover)",
                    border: "1px solid var(--border)",
                    borderRadius: 8,
